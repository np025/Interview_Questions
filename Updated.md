...
```
```
78. What would you do if an EC2 instance is getting slow?
```
```
79. If users can’t access an application hosted on EC2, what steps would you take?
```
```
80. What’s the difference between a Load Balancer and a Reverse Proxy?
```
```
81. How would you write a Terraform script to create an EC2 instance and run a script on every reboot?
```
```
82. What is a Backend in Terraform, and why is it used?
```
```
83. What is the Docker lifecycle?
```
```
84. What are the key Docker components?
```
```
85. What’s the difference between a Docker Image and a Docker Container?
```
```
86. What should you do before creating a Docker container?
```
```
87. What is Docker Compose, and how do you use it?
```
```
88. What steps would you take if you see an "unhealthy" status in an ELB?
```
```
89. How do you optimize Docker images for better performance?
```
```
90. How would you secure a Docker container?
```
```
91. What is Jenkins scaling, and how do you achieve it?
```
```
92. What is the role of the Master and Node in Jenkins?
```
```
93. What is a Sidecar container, and when would you use it?
```
```
94. What is the difference between ConfigMap and Secrets in Kubernetes?
```
```
95. What is the default deployment in Kubernetes?
```
```
96. What are Taints and Tolerations in Kubernetes?
```
```
97. What is a Static Pod in Kubernetes, and how is it different from a regular pod?
```
```
98.  How do you check pod logs and attach Prometheus for monitoring?
```
```
99. How would you define a ConfigMap and Secrets in Kubernetes?
```
```
100. What is the default scaling in Kubernetes, and how does it work?
```
```
101. What is RBAC in Kubernetes, and why is it important?
```
```
102. What’s the difference between ClusterRole and Role in RBAC?
```
